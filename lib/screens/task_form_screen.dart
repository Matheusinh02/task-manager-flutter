...
class _TaskFormScreenState extends State<TaskFormScreen> {
  ...
  Future<void> _takePicture() async {
    ...
    if (photoPath != null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('📷 Foto ${_photoPaths.length} capturada!'),
  ...
  Future<void> _pickFromGallery() async {
    ...
    if (photoPath != null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('📷 Foto ${_photoPaths.length} adicionada!'),
          backgroundColor: Colors.green,
          duration: const Duration(seconds: 2),
        ),
      );
  ...
  Future<void> _pickMultipleFromGallery() async {
    final photoPaths = await CameraService.instance.pickMultipleFromGallery(context);

    if (photoPaths.isNotEmpty && mounted) {
      setState(() => _photoPaths.addAll(photoPaths));

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('📷 ${photoPaths.length} foto(s) adicionadas! Total: ${_photoPaths.length}'),
          backgroundColor: Colors.green,
          duration: const Duration(seconds: 2),
        ),
      );
